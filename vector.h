 ...
 * elements, which means that their elements can also be accessed using offsets
 ...
 * when new elements are inserted, which implies allocating a new array and
 * moving all elements to it.  This is a relatively expensive task in terms of
 * processing time, and thus, vectors do not reallocate each time an element is
 ...
 * last element in the vector. It does not point to any element, and thus shall
 * not be dereferenced.
 *
 ...
 * 'vector'.  When this capacity is exhausted and more is needed, it is
 ...
 * elements in the vector, throwing an assertion (assert.h) if it is not (i.e.,
 * if n is greater than, or equal to, its size).
 *
 ...
 * element. This effectively increases the container size by one, which causes
 * an automatic reallocation of the allocated storage space if -and only if- the
 ...
 *
 * Because vectors use an array as their underlying storage, erasing elements in
 * positions other than the vector end causes the container to relocate all the
 * elements after the segment erased to their new positions.  This is generally
